...
import { CameraPlus, CameraLoadedEvent, ImagesSelectedEvent, PhotoCapturedEvent, ToggleCameraEvent } from '@nstudio/nativescript-camera-plus';

@Component({
	selector: 'demo-nativescript-camera-plus',
	templateUrl: 'nativescript-camera-plus.component.html',
})
export class NativescriptCameraPlusComponent {
	demoShared: DemoSharedNativescriptCameraPlus;

	ngOnInit() {
		this.demoShared = new DemoSharedNativescriptCameraPlus(this.page);
	}

	// TODO: condense the below to use the shared demo code above to reduce plugin demo maintenance costs
	// was dropped in "as-is" from original repo for now (n.walker - 12/28/2020)
