...
export class AirshipPush implements IAirshipPush {
    ...
    enable() {
        return new Promise<boolean>((resolve, reject) => {
            ...
            resolve(com.urbanairship.UAirship.shared().getPrivacyManager().isEnabled(
                [
                    com.urbanairship.PrivacyManager.FEATURE_PUSH
                ]
            ));
        })
    }
...
export class AirshipConfig implements IAirshipConfig {
    ...
    static defaultConfig() {
        ...
        config._native = new com.urbanairship.AirshipConfigOptions.Builder().applyDefaultProperties(
...
export class Airship implements IAirship {
    ...
    static takeOff(config?: AirshipConfig, onReady?: (airship: Airship) => void) {
        const cb = () => {
            if (config) {
                if (onReady) {
                    com.urbanairship.UAirship.takeOff(Utils.android.getApplicationContext(), config._native.build(), new com.urbanairship.UAirship.OnReadyCallback({
                ...
                if (onReady) {
                    com.urbanairship.UAirship.takeOff(Utils.android.getApplicationContext(), AirshipConfig.defaultConfig()._native.build(), new com.urbanairship.UAirship.OnReadyCallback({
                        onAirshipReady(param0: com.urbanairship.UAirship) {
                            Airship._nativeInstance = param0;
                            onReady(Airship.getInstance());
                        }
