		...
		const requestImg = (i: number) => {
			...
			if (asset.mediaType === PHAssetMediaType.Image) {
				...
				if (manager.requestImageDataAndOrientationForAssetOptionsResultHandler) {
					manager.requestImageDataAndOrientationForAssetOptionsResultHandler(asset, requestOptions, (imageData: NSData, dataUti: string, orientation: CGImagePropertyOrientation, info: NSDictionary<any, any>) => {
					...
					manager.requestImageForAssetTargetSizeContentModeOptionsResultHandler(asset, targetSize, PHImageContentMode.AspectFill, requestOptions, (image: UIImage, info: NSDictionary<any, any>) => {
						const imageAsset = new ImageAsset(image);
						if (this._width) imageAsset.options.width = this._width;
						if (this._height) imageAsset.options.height = this._height;
						imageAsset.options.keepAspectRatio = this._keepAspectRatio;
				...
				manager.requestAVAssetForVideoOptionsResultHandler(asset, requestOptions, (videoAsset: AVAsset, audioMix, info) => {
					if (videoAsset.isKindOfClass(AVURLAsset.class())) {
						...
						const result = NSFileManager.defaultManager.copyItemAtURLToURLError((<AVURLAsset>videoAsset).URL, targetURL);
