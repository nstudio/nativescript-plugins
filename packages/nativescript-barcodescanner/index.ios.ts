...
import { BarcodeFormat, BarcodeScannerView as BarcodeScannerBaseView, ScanOptions, ScanResult } from './common';
...
export class BarcodeScannerView extends BarcodeScannerBaseView {
	...
	initView() {
		...
		this._delegate.setCallback(this.beepOnScan, true, this.reportDuplicates, this.formats, (text: string, format: string) => {
			that.notify({
	...
	public scan(arg: ScanOptions): Promise<ScanResult> {
		return new Promise((resolve, reject) => {
			try {
				...
				this._scanDelegate.setCallback(arg.beepOnScan !== false, isContinuous, arg.reportDuplicates, arg.formats, (text: string, barcodeFormat: BarcodeFormat) => {
