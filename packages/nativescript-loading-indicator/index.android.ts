...
export class LoadingIndicator {
	...
	private _createPopOver(context, options?: OptionsCommon) {
		...
		console.log('dimming the background...', options.dimBackground);
		...
		if (options.hideBezel) {
			...
			parentView.setBackground(border);
		}
		...
		switch (options.mode) {
			...
			case Mode.AnnularDeterminate:
				...
				progressView.setBackground(this._getBackgroundDrawable());
	...
	private _getResources() {
		const ctx: android.app.Activity = Application.android.foregroundActivity || Application.android.startActivity;
	...
	private _getBackgroundDrawable() {
		if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.LOLLIPOP) {
			// >= API 21
			return this._getResources().getDrawable(com.github.triniwiz.ns.loading.indicator.R.drawable.circle_shape, Utils.android.getApplicationContext().getTheme());
		} else {
	...
	private _getProgressDrawable() {
		if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.LOLLIPOP) {
			// >= API 21
			return this._getResources().getDrawable(com.github.triniwiz.ns.loading.indicator.R.drawable.circular_progress_bar, Utils.android.getApplicationContext().getTheme());
		} else {
	...
	private _getProgressDrawableThick() {
		if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.LOLLIPOP) {
			// >= API 21
			return this._getResources().getDrawable(com.github.triniwiz.ns.loading.indicator.R.drawable.circular_progress_bar_thick, Utils.android.getApplicationContext().getTheme());
		} else {
