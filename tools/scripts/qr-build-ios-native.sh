#!/bin/sh
cd ../..
CWD="$(pwd)/packages/nativescript-qr/src-native"
NATIVE_SRC="$CWD/qr-native"
IOS_SRC_DIR="$CWD/qr-ios"
IOS_LIB_DIR="$IOS_SRC_DIR/QrNative"
...
if [[ -f "$IOS_LIB_INCLUDE/qr_native.h" ]]; then
  rm "$IOS_LIB_INCLUDE/qr_native.h"
fi

# TODO fix header generation .... ignore android
cbindgen --config "$CWD/qr-native/cbindgen.toml"  "$CWD/qr-native/src/lib.rs" -l c >"$IOS_LIB_INCLUDE/qr_native.h"
